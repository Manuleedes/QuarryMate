  ...
  return (
    <Card className=" m-3">
      <CardHeader className="text-sm"
        action={
          <IconButton onClick={()=>navigate(`/quarry/${item.quarry.address.city}/${item.quarry.name}/${item.quarry.id}`)} aria-label="settings">
            <EastIcon />
          </IconButton>
        }
        title={<p className="text-base"> {item.quarry?.name} </p>}
